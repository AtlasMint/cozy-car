...
describe('prefs persistence', () => {
  test('sanitize drops malformed values and clamps volume', () => {
    expect(sanitizePrefs({ mode: 'focus', masterVolume: 1.7, reducedMotion: 'yes', quality: 'ultra' })).toEqual({ mode: 'focus', masterVolume: 1 });
  ...
  test('round-trips through a storage shim', () => {
    ...
    savePrefs({ mode: 'focus', masterVolume: 0.4, reducedMotion: true, quality: 'low' }, storage);
    expect(mem.has(PERSIST_KEY)).toBe(true);
    expect(loadPrefs(storage)).toEqual({ mode: 'focus', masterVolume: 0.4, reducedMotion: true, quality: 'low' });
  });
